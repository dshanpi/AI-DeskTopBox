...
![智能桌面小咪外观渲染](../assets/product-front.png)

> 上图为当前产品形态基准渲染；最终量产细节以冻结后的实物为准。
...
| 层级 | 核心组成 | 主要职责 |
| --- | --- | --- |
| L4 体验与应用层 | LVGL 桌面、AI 对话、HDMI 预览、浏览器 IPKVM、用户 APP | 向用户呈现功能入口、任务过程和设备状态，承载品牌与行业差异化 |
| L3 平台服务层 | UI 后端、应用管理、MCP/Agent、硬件控制、云连接、OTA 与设备身份 | 统一调度独立进程，控制权限和硬件访问，管理设备完整生命周期 |
| L2 系统与驱动层 | Linux 4.9、Tina/OpenWrt、ALSA、V4L2、网络与 USB Gadget | 提供显示、音频、视频、网络、输入和 USB 设备模式等稳定系统接口 |
| L1 硬件与接口层 | A133、触控显示、麦克风、扬声器、HDMI、Wi-Fi/蓝牙、USB、WS2812 | 负责声音与画面感知、屏幕/音频/灯效表达，以及授权键鼠动作执行 |

这种分层将板级驱动、通用平台能力和客户应用分开：硬件变更主要收敛在 L1/L2，
品牌体验集中在 L4，量产安全与运维策略由 L3 统一约束。应用和服务采用独立进程，
避免单个扩展模块直接影响桌面主循环。
...
1. 麦克风通过 ALSA 采集音频，AEC 降低扬声器回声对识别的影响。
2. 唤醒与语音前处理确认有效输入后，将请求交给 AI 运行时。
3. 云端模型完成识别、理解和回复生成，角色、音色与设备配置参与生成过程。
4. 设备流式显示文字并播放语音，同时用屏幕动画和 WS2812 表达聆听、思考和说话状态。

**电脑视觉与控制链路**

1. HDMI 输入提供目标电脑画面，本机可直接预览或截取静态帧。
2. 硬件 H.264 编码与 WebRTC 向授权浏览器传输画面；需要自动化时，画面可作为 AI 视觉输入。
3. 控制策略检查用户授权、任务范围、动作上限与急停状态。
4. 通过标准 USB HID 输出键盘和鼠标动作，任务结束或异常时自动释放按键。

### 三类使用入口

- **本机入口**：触摸、遥控和语音完成日常交互、设备设置与状态查看。
- **手机入口**：完成配网、绑定、在线状态查看，以及角色、音色等设备面板配置。
- **浏览器入口**：用于 IPKVM 远程控制、扩展应用管理和设备运维；具体开放范围由交付版本决定。
